# 🏨 Aloja - Sistema de Gestión Hotelera

**Aloja** es un sistema de administración hotelera desarrollado con **PHP**, **MySQL** y **JavaScript**, diseñado para gestionar habitaciones, huéspedes, empleados, tarifas, pagos y más.


---

## 🚀 Funcionalidades Principales

- 🔐 Inicio de sesión seguro con manejo de sesiones.
- 👥 Registro, edición y eliminación de empleados y huéspedes.
- 🛏️ Gestión completa de habitaciones.
- 💵 Registro y visualización de pagos.
- 📊 Control de tarifas, novedades e ingresos.
- 📍 Visualización de ubicación de habitaciones.

---

## 🧰 Tecnologías Utilizadas

- **Frontend:** HTML, CSS, JavaScript
- **Backend:** PHP
- **Base de Datos:** MySQL
- **Servidor Local:** XAMPP
- **Control de versiones:** Git + GitHub

---

📌 Estado del Proyecto
Este proyecto está en desarrollo activo. Próximas funciones a implementar:

 Roles de usuario (admin y empleado)

 Validaciones en formularios

 Mejoras visuales con TailwindCSS

👤 Autor
Miguel Botero
📧 mabotero0506@gmail.com
🔗 GitHub: MiguelBotero


