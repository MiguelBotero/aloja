            ...
            <?php while ($row = mysqli_fetch_assoc($resultado)) { ?>
                <tr>
                    ...
                    <td class="flex flex-wrap gap-2">
                        <a class="bg-yellow-400 hover:bg-yellow-500 text-black px-4 py-1 rounded-full text-sm font-semibold shadow-md transition-transform hover:scale-105" href="../administrador/editar_otroingresos.php?id=<?= $row['id_otroingreso']?>"  style="">Editar</a>
                        <a class="bg-red-500 hover:bg-red-600 text-white px-4 py-1 rounded-full text-sm font-semibold shadow-md transition-transform hover:scale-105" href="../php/eliminar_otroingreso.php?id=<?= $row['id_otroingreso']?>" onclick="return confirm('¿Estas seguro que deseas eliminar este registro?');">eliminar</a>
                    </td>
                </tr>
            <?php } ?>
        </tbody>
